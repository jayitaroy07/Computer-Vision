...
def histeq(image):
    ...
    # calHist(input,channels,mask,histogram size,range)
...
target_luv=cv2.cvtColor(target,cv2.COLOR_BGR2Luv)
lmatrix=target_luv[:,:,0]
# target_hist=lscl(lmatrix)
# target_hist=cv2.equalizeHist(lmatrix)
target_hist=histeq(lmatrix)
target_luv[:,:,0]=target_hist
target_bgr=cv2.cvtColor(target_luv,cv2.COLOR_Luv2BGR)
